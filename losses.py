...
L_total = L_task
        + lambda_util   * L_util    (prevent dead vectors)
        + lambda_div    * L_div     (prevent key collapse)
        + lambda_norm   * L_norm    (prevent assembly explosion)
        + lambda_sparse * L_sparse  (encourage sparse assembly weights)
"""
...
def utilization_loss(alpha_ema: jax.Array, beta: float = 0.1) -> jax.Array:
    """
    Prevent dead vectors: L_util = -mean_i log(1 - exp(-β * EMA(alpha_i)))

    Vectors with low EMA usage contribute large loss, pushing the model to
    use them. EMA(alpha_i) is the exponential moving average of per-vector
...
def diversity_loss(alpha: jax.Array, keys: jax.Array) -> jax.Array:
    ...
    is O(N^2) regardless of batch size — safe for sequence models where
    ...
    # Outer product of mean utilisation: [N, N]  — O(N^2), batch-independent
...
def norm_loss(W_assembled: jax.Array, W_base: jax.Array) -> jax.Array:
    """
    Prevent assembly explosion: L_norm = mean_batch ||W_assembled - W_base||^2_F
...
def sparsity_loss(alpha: jax.Array) -> jax.Array:
    """
    Weight entropy: L_sparse = -mean_batch sum_i alpha_i log(alpha_i)
